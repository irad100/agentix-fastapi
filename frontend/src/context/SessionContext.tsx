...
export const SessionProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  const { userToken, isAuthenticated } = useAuth(); // Get user token and auth status
  ...
  let createNewSessionFn: () => Promise<SessionInfo | null>;
  ...
  }, [userToken, switchActiveSession]); // Dependency on userToken and the setter

  createNewSessionFn = useCallback(async (): Promise<SessionInfo | null> => {
      if (!userToken) { 
  ...
  }, [userToken, fetchSessions]); // React to userToken changes
